<template>
    <div class="nait-chat-interface mx-auto flex flex-col transition-all duration-500 ease-in-out"
        :style="{ maxHeight: isChatActive ? 'calc(100dvh - var(--vp-nav-height, 0px))' : 'auto' }"
        :class="[
            isChatActive ? 'h-[calc(100dvh-var(--vp-nav-height,0px))] overflow-hidden relative z-60' : 'py-8'
        ]">
...
const updateCaretVisibility = (type: "initial" | "active") => {
	const scrollContainer =
		type === "initial"
			? initialPromptsScrollRef.value
			: activePromptsScrollRef.value;
	const showLeftCaret =
		type === "initial" ? showInitialLeftCaret : showActiveLeftCaret;
	const showRightCaret =
		type === "initial" ? showInitialRightCaret : showActiveRightCaret;
...
const onMouseMove = (type: "initial" | "active", event: MouseEvent) => {
	const scrollContainer =
		type === "initial"
			...
			: activePromptsScrollRef.value;
	const isDragging =
		type === "initial" ? isDraggingInitial.value : isDraggingActive.value;
	const startX = type === "initial" ? startXInitial.value : startXActive.value;
	const scrollLeft =
		type === "initial" ? scrollLeftInitial.value : scrollLeftActive.value;
...
const onTouchMove = (type: "initial" | "active", event: TouchEvent) => {
	const scrollContainer =
		type === "initial"
			...
			: activePromptsScrollRef.value;
	const isDraggingRef =
		type === "initial" ? isDraggingInitial : isDraggingActive;
	const startXValue =
		type === "initial" ? startXInitial.value : startXActive.value;
	const scrollLeftValue =
		type === "initial" ? scrollLeftInitial.value : scrollLeftActive.value;

	if (!isDraggingRef.value || !scrollContainer || event.touches.length === 0)
		return;
...
const closeChatSession = () => {
	...
	console.log(
		`NaitChat: Chat view closed. Session ${sessionId.value} remains available.`,
	);
	...
	nextTick(() => {
		...
		if (initialTextareaRef.value)
			adjustTextareaHeight(initialTextareaRef.value); // Still adjust height as userInput is cleared.
	...
	typingInterval = setInterval(() => {
		...
		if (chatMessages.value[messageIndex]) {
			chatMessages.value[messageIndex].text =
				"Nait is thinking" + ".".repeat(dotCount);
		} else {
			stopTypingIndicator();
		}
	}, 500);
...
const sendMessage = async () => {
	...
	try {
		...
		if (
			...
			// Fallback for local dev if VITE_API_HOST is not set on window
			url = "http://localhost:3000/api/nait/chat";
			console.warn(
				"NaitChat: VITE_API_HOST not found on window, using default dev URL:",
				url,
			);
		} else if (!apiHost) {
		...
		if (!response.ok) {
			...
			if (chatMessages.value[naitMessageIndex]?.id === naitMessageId) {
				chatMessages.value[naitMessageIndex].text =
					`Sorry, Nait encountered an error: ${errorData.details || errorData.error || "Server error"}`;
			}
			return;
		}
		...
		if (chatMessages.value[naitMessageIndex]?.id === naitMessageId) {
			chatMessages.value[naitMessageIndex].text =
				"Sorry, there was an issue connecting to Nait.";
		} else {
			chatMessages.value.push({
				id: `fetch_err_${Date.now()}`,
				text: "Could not connect to Nait.",
				isNait: true,
			});
		}
	} finally {
