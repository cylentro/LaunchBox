...
const filteredCourses = computed(() => {
  ...
  if (selectedSeries.value) {
    return allCourses
      .filter(course => {
        ...
        const seriesMatch = course.series && course.series.name === selectedSeries.value;
  ...
  const pinnedCourses = allCourses.filter(course => course.pinOrder !== undefined && course.pinOrder !== null).sort((a, b) => a.pinOrder - b.pinOrder).slice(0, 3);
...
function getCourseByLink(link) {
  return courses.value.find(c => c.link === link);
}
