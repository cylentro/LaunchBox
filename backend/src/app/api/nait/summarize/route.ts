...
import { HumanMessage, SystemMessage, BaseMessageChunk } from '@langchain/core/messages';
...
export async function POST(request: Request) {
  try {
    ...
    if (!pagePath || typeof pagePath !== 'string') {
      return NextResponse.json({ error: "Invalid 'pagePath' provided. It must be a string." }, { status: 400 });
    }
    ...
    if (!fs.existsSync(fullFilePath)) {
      ...
      return NextResponse.json({ error: `Content for page '${pagePath}' not found.` }, { status: 404 });
    }
    ...
    try {
      ...
      if (docs.length > 0 && docs[0].pageContent) {
        ...
        console.error(`Summarize Error: No content loaded from ${fullFilePath}`);
        return NextResponse.json({ error: `Could not load content from '${pagePath}'.` }, { status: 500 });
      }
    } catch (loadError) {
      console.error(`Summarize Error: Error loading file ${fullFilePath}:`, loadError);
      return NextResponse.json({ error: `Error processing page content for '${pagePath}'.` }, { status: 500 });
    }

    if (documentContent.trim() === "") {
      documentContent = "This page appears to be empty or contains only whitespace.";
    }
...
const fileNameWithoutExt = path.parse(path.basename(pagePath)).name;
    ...
    let formattedPageName = words
      ...
      .join(' ');
    if (!formattedPageName && fileNameWithoutExt) { // Fallback for empty result (e.g. if filename was just "-")
        formattedPageName = fileNameWithoutExt.charAt(0).toUpperCase() + fileNameWithoutExt.slice(1);
    }
...
5.  If the provided text is very short (e.g., "This page appears to be empty...") or lacks clear substance for a detailed summary, then after the required starting phrase "Summary of ${formattedPageName}: ", you should state something like "This page contains minimal content for a detailed summary." or "This page appears to be empty."`;
    ...
    const messagesForLlm = [
      ...
      new HumanMessage(`Document content to summarize:\n\n---\n${documentContent}\n---`),
    ];
    ...
    const readableStream = new ReadableStream({
      async start(controller) {
        ...
        controller.enqueue(encoder.encode(`data: ${JSON.stringify({ type: "summary_end" })}\n\n`));
        controller.close();
      },
      cancel() {
    ...
    return NextResponse.json({ error: "Nait is currently unavailable for summarization.", details: errorMessage }, { status: 500 });
  }
}